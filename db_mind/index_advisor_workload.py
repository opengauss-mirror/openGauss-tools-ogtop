...
try:
    ...
    from db_mind.sql_output_parser import parse_single_advisor_results, parse_explain_plan, \
        get_checked_indexes, parse_table_sql_results, parse_existing_indexes_results, parse_plan_cost, parse_hypo_index
    from db_mind.sql_generator import get_single_advisor_sql, get_index_check_sqls, get_existing_index_sql, \
        get_workload_cost_sqls, get_index_setting_sqls, get_prepare_sqls, get_hypo_index_head_sqls
    from db_mind.executors.common import BaseExecutor
    from db_mind.executors.gsql_executor import GsqlExecutor
    from db_mind.mcts import MCTS
    from db_mind.table import get_table_context
    from db_mind.utils import match_table_name, IndexItemFactory, \
        AdvisedIndex, ExistingIndex, QueryItem, WorkLoad, QueryType, IndexType, COLUMN_DELIMITER, \
        lookfor_subsets_configs, has_dollar_placeholder, generate_placeholder_indexes, \
        match_columns, infer_workload_benefit, UniqueList, is_multi_node, hypo_index_ctx, split_iter, \
        replace_comma_with_dollar, replace_function_comma, flatten, ERROR_KEYWORD
    from db_mind.process_bar import bar_print, ProcessBar
...
def main(argv):
    ...
    arg_parser.add_argument(
        "--db_mind-host", "--h", help="Host for database", action=CheckWordValid)
    arg_parser.add_argument(
        "-U", "--db_mind-user", help="Username for database log-in", action=CheckWordValid)
    arg_parser.add_argument(
