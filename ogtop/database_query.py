...
class DatabaseMain(object):
    ...
    def __init__(self, openGauss_option, sqls, params):
        ...
        """
        database_data = openGauss_option.query_sql(sqls, (params,))
        if isinstance(database_data, str):
            ...
            if len(database_data) == 54:
                self.de_part = {
                    ...
                    "state": database_data[14] if database_data[14] else "-",  # 11
                    "LocalRoll": database_data[15] if database_data[15] else "single",
                    "ProcessMem": database_data[45] if len(database_data) == 54 and database_data[45] != None else '-',
                    "DynamicMem": database_data[47] if len(database_data) == 54 and database_data[47] != None else '-',
                    "SharedMem": database_data[48] if len(database_data) == 54 and database_data[48] != None else '-',
                    ...
                    "pgwrits": database_data[21] if database_data[21] != "()" else 0,
                    ...
                    "os_load_processes": float(database_data[13]) if database_data[13] else 0,  # load
                }
            else:
                self.de_part = {
                    "Start_Date": datetime.datetime.strftime(database_data[0], "%Y-%m-%d"),
                    ...
                    "DB_TIME": database_data[17] if database_data[17] else 0,
                    "CPU_TIME": int(database_data[18]) if database_data[18] else 0,
                    "EXECUTION_TIME": int(database_data[19]) if database_data[
                        19] else 0,
                    "DATA_IO_TIME": int(database_data[16]) if database_data[
                        16] else 0,
                    ...
                    "temp_files": database_data[34] if database_data[34] else 0,
                    "temp_bytes": database_data[35] if database_data[35] else 0,
                    "conflicts": int("".join(list(filter(str.isdigit, database_data[33])))) if database_data[33] else 0,
                    "deadlocks": database_data[36] if database_data[36] else 0,
                    "checkpoints": database_data[21] if database_data[21] else 0,
    ...
    def compute_delta(old_ob, new_ob):
        ...
        delta_de = {
            ...
            "pgwrits": 'None' if new_ob.de_part['pgwrits'] == None else round(
                new_ob.de_part['pgwrits'] - old_ob.de_part['pgwrits']),
        ...
        delta_up = {
            ...
            "DATA_IO_TIME/DB_TIME": 0.0 if delta_de['data_io_time'] == 0 else delta_de['data_io_time'] / delta_de[
                'DB_TIME'] * 100,
            "dynamic_used_memory/max_dynamic_memory": '-' if delta_de['max_dynamic_memory'] == '-' else delta_de[
                                                                                                            'Dynamic_Mem'] /
                                                                                                        delta_de[
